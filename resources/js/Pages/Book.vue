        ...
        <div class="mx-auto max-w-screen-xl p-6">
            ...
            <div class="mt-6 rounded-2xl bg-white p-8 shadow-lg dark:bg-gray-800">
                <div class="flex flex-col gap-8 md:flex-row">
                    <!-- Cover -->
                    <img :src="book.cover_image || 'https://via.placeholder.com/200x300?text=No+Cover'" :alt="book.title"
                        class="w-full rounded-xl object-cover shadow md:w-1/3" />

                    <!-- Info -->
                    <div class="flex-1">
                        <h1 class="mb-1 text-4xl font-bold">{{ book.title }}</h1>
                        <p class="mb-4 text-lg text-[#706f6c] dark:text-[#A1A09A]">by {{ book.author }}</p>

                        <!-- Rating -->
                        <div class="mb-6 flex items-center">
                            <svg v-for="n in 5" :key="n" class="h-6 w-6" :class="n <= Math.round(book.average_rating)
                                ? 'text-yellow-400'
                        ...
                        <dl class="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
                            ...
                            <!-- Check out -->
                            <button v-if="book.is_available" @click="checkout"
                                class="rounded bg-blue-600 px-4 py-2 text-white">
                                Check out for 5&nbsp;days
                            </button>

                            <button v-else-if="$page.props.auth.user?.role === 'Librarian'" @click="markReturned"
